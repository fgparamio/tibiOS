...
`RerankProvider` is *wired* (design decision D18): same shape as
`ChatProvider`/`EmbeddingProvider` — two constructor-injected, immutable
fields, `backends` and `selection_policy`, normalized via
`MappingProxyType` in `__post_init__`. `execute()` runs the same
ADR-0002 flow through `capabilities/dispatch.py`'s module-level pure
functions and `capabilities/requests.py`'s `RerankRequest` — this module
holds only what is rerank-specific: calling `RerankBackend.rerank()` once
(a batch call, not a token stream) and the D24 rerank codec.

Rerank codec (D24): exactly **one** `OutputChunk`, `sequence=0`, UTF-8
JSON `{"results": [{"index": i, "score": s}, ...]}` — one object per
`RerankResult` returned by `rerank()`, in the order `rerank()` returned
them; `RerankResult.index` refers back into the original `documents`
sequence, not the position in this list. Unlike `chat.py`'s per-delta
codec there is no "empty/terminal" filter to apply: a batch result is
either emitted whole or not at all, so the D24 codec itself introduces no
conditional. The one conditional native to this module's `execute()` is
cooperative cancellation — checked once, after `rerank()` resolves and
the session is released, since a batch capability method is a single
awaited call with no intermediate progress to poll mid-flight
(`provider-backend-composition` spec: "Cooperative cancellation is
observed mid-execution" — for a batch Provider, "stops driving further
output" means the one batch result is not emitted).
...
from tibios_ray.capabilities.requests import RerankRequest
...
class RerankProvider:
    ...
    async def execute(self, context: ExecutionContext) -> ExecutionReport:
        ...
        request = RerankRequest.parse(context.execution_parameters)
        ...
        try:
            results = await backend.rerank(session, request.query, request.documents)
        except Exception as error:
            raise BackendExecutionError(
