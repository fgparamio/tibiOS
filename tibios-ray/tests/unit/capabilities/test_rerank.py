...
none"; `provider-backend-composition` spec: "Per-Request Dispatch Flow",
"Backend Session Release Is Guaranteed", "Non-Streaming Results Travel
Through the Channel").
...
`RerankProvider`: successful dispatch, the D24 rerank codec, the release
guarantee, failure outcomes, cancellation, and the dispatch-mechanical-
only conditional shape of `execute()` (tasks 5.5, 5.6, 5.9) — same
coverage shape as `test_embedding.py`, rerank-specific.
...
# Never actually consulted: only used to satisfy RerankProvider's required
...
def _rerank_execution_parameters(**overrides: str) -> dict[str, str]:
    parameters = {
        "query": "what is tibios",
        "documents": json.dumps(["doc a", "doc b"]),
    }
...
class TestRerankProvider:
    ...
    def test_flags_are_all_false(self) -> None:
        flags = RerankProvider(backends={}, selection_policy=_UNRESOLVED_POLICY).descriptor.flags
...
class TestRerankProviderDispatch:
    """Task 5.5: successful dispatch acquires with the resolved plan,
    drives `rerank()`, releases the session, emits the D24 batch chunk,
    ...
    def test_successful_dispatch_emits_output_and_completes(self) -> None:
        ...
        context = FakeExecutionContext(
            execution_parameters=_rerank_execution_parameters(),
        ...
        assert backend.rerank_calls[0][0] is acquired_session
    ...
    def test_rerank_is_called_with_the_parsed_rerank_request(self) -> None:
        backend = FakeRerankBackend(_ONNXRUNTIME, results=(RerankResult(index=0, score=1.0),))
        plan = _plan()
        provider = RerankProvider(
            backends={_ONNXRUNTIME: backend},
            selection_policy=FakeModelSelectionPolicy(plan=plan),
        ...
        _, query, documents = backend.rerank_calls[0]
        assert query == "what is tibios"
        assert tuple(documents) == ("doc a", "doc b")


class TestRerankCodec:
    """Task 5.6: the D24 rerank codec — exactly one `OutputChunk`,
    `sequence=0`, `json.loads(data)` round-trips `{"results": [{"index":
    i, "score": s}, ...]}` in order; `ExecutionReport` carries none of the
    scores (`provider-backend-composition` spec: "Rerank output appears
    on the channel, not the report")."""

    def test_one_chunk_sequence_zero_json_round_trips_results_in_order(self) -> None:
        results = (RerankResult(index=1, score=0.9), RerankResult(index=0, score=0.2))
        backend = FakeRerankBackend(_ONNXRUNTIME, results=results)
        plan = _plan()
        provider = RerankProvider(
            backends={_ONNXRUNTIME: backend},
            selection_policy=FakeModelSelectionPolicy(plan=plan),
        ...
        # failure); asserted here by inspecting the two Mapping fields
        # that could smuggle a score value in.
        assert 0.9 not in report.resource_usage.values()
        assert 0.9 not in report.metrics.values()


class _CancelBeforeCompleteBackend:
    """Local fake conforming to `RerankBackend`: cancels the caller-
    supplied token from inside `rerank()`, simulating
    ...
    method is a single awaited call with no intermediate results, the
...
class TestRerankProviderCancellation:
    """Task 5.5: cooperative cancellation observed once the Backend's
    `rerank()` resolves stops the batch result from being emitted, still
    releases the acquired session, and returns `CANCELLED` without
    raising."""

    def test_cancellation_after_rerank_resolves_skips_emission_releases_and_returns_cancelled(
        self,
    ) -> None:
        cancellation = ManualCancellation()
        backend = _CancelBeforeCompleteBackend(
            _ONNXRUNTIME, cancellation, results=(RerankResult(index=0, score=1.0),)
        )
        plan = _plan()
        provider = RerankProvider(
            backends={_ONNXRUNTIME: backend},
...
class TestRerankProviderReleaseGuarantee:
    """Task 5.5: `release()` is called exactly once for every successful
    `acquire()`, including when `rerank()` raises mid-execution;
    `release()` is never called when `acquire()` itself raises
    (`provider-backend-composition` spec: "Backend Session Release Is
    Guaranteed")."""

    def test_release_is_called_when_rerank_raises(self) -> None:
        backend = FakeRerankBackend(_ONNXRUNTIME, rerank_raises=RuntimeError("boom"))
        plan = _plan()
        provider = RerankProvider(
            backends={_ONNXRUNTIME: backend},
            selection_policy=FakeModelSelectionPolicy(plan=plan),
    ...
    def test_release_is_never_called_when_acquire_raises(self) -> None:
        backend = FakeRerankBackend(_ONNXRUNTIME, acquire_raises=RuntimeError("no residency"))
        plan = _plan()
        provider = RerankProvider(
            backends={_ONNXRUNTIME: backend},
            selection_policy=FakeModelSelectionPolicy(plan=plan),
...
class TestRerankProviderFailureOutcomes:
    """Task 5.5: an empty mapping fails as `NoBackendAvailableError`; a
    ...
    def test_empty_mapping_raises_no_backend_available_error(self) -> None:
        ...
        assert exc_info.value.capability == RERANK_DOCUMENTS_DESCRIPTOR.capability
        assert exc_info.value.provider == "RerankProvider"
    ...
    def test_plan_naming_a_backend_absent_from_a_non_empty_mapping_raises_unresolvable(
        ...
        provider = RerankProvider(
            backends={present_backend_id: FakeRerankBackend(present_backend_id)},
            selection_policy=FakeModelSelectionPolicy(plan=plan),
        )
        context = FakeExecutionContext(
            execution_parameters=_rerank_execution_parameters(),
...
class TestRerankProviderDispatchMechanicalConditionals:
    """Task 5.9: the only conditional native to `RerankProvider.execute()`'s
    ...
    def test_every_conditional_in_execute_is_the_allowed_cancellation_check(self) -> None:
        source = textwrap.dedent(inspect.getsource(RerankProvider.execute))
        ...
        for condition in found_conditions:
            assert condition in self._ALLOWED_CONDITIONS, (
                f"unexpected conditional {condition!r} in RerankProvider.execute() — "
            ...
            if isinstance(node, ast.Compare):
                ...
                for banned in banned_names:
                    assert banned not in unparsed, (
                        ...
                        f"RerankProvider.execute(): {unparsed!r} — this looks like "
