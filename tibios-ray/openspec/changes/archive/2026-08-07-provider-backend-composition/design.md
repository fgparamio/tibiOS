...
- **ADR-0003 defines backend initialization as the lifetime owner of heavyweight resources. Current `vllm.py` and `onnxrt.py` implementations perform lazy initialization on first `acquire()` (VL2/OR2) because the Backend initialization lifecycle currently exposes no explicit warmup hook — this is a temporary implementation limitation, not an architectural exception.** Only `llamacpp.py` is changed here, because ADR-0003 names it specifically and `proposal.md` puts the other engines out of scope; closing the gap for the other two needs a `warmup()` seam on `BackendAdapter` — an eager `acquire()`+`release()` at boot cannot substitute for it, since refcount-zero release tears the resource straight back down (VL13/OR2). This does not reinterpret or weaken ADR-0003: the invariant still holds as written, it is simply not yet mechanically enforceable for these two engines. Recorded in Open Questions, not hidden — resolved by a future `backend-lifecycle`/`backend-warmup` change that adds `initialize()`/`warmup()`/`shutdown()` to `BackendAdapter` and lets the Composition Root call `warmup()` on every Backend before Providers are constructed.
...
- **D29** hit the same class of gap once Slice 7 actually wired `worker.py` through `engines/__init__.py`'s pre-existing package-level re-export: "Composition Root Exclusive Backend Ownership"'s original scenario asked *"only `worker.py` imports them"*, which an `ast`-level import scan finds literally false — `engines/__init__.py` also names each class, in order to re-export it. The invariant that requirement actually protects was never about who imports a name; it's that only the Composition Root decides which concrete implementation gets constructed and wired into a Provider. Corrected `provider-backend-composition/spec.md` to state that directly — "the only module that constructs a concrete Backend/engine instance or wires one into a Provider," with an explicit carve-out for package-level re-exports whose sole purpose is API aliasing — and the guard test now scans for constructor *calls*, not import statements, so it survives `engines/`'s internal layout changing without another spec revision, the same way D26's pool survives a future change in count.

## Open Questions

- [ ] **ADR-0003 is only mechanically enforced for `llamacpp.py`; vLLM/ONNX are a temporary implementation gap, not an architectural exception (see "Accepted, explicit limitations").** Needs a `warmup()` seam on `BackendAdapter` — a `backends/` contract change with its own spec delta, tracked as a future `backend-lifecycle`/`backend-warmup` change.
