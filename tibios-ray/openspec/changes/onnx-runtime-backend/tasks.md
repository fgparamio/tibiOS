...
- [ ] 1.3 Create `tests/unit/engines/stub_onnx.py`: `StubInferenceSession`, `StubNodeArg`, `StubTokenizer` with call-recording factories (`stub_llama.py`/`stub_async_llm.py` precedent).
- [ ] 1.4 RED: SDK-free import test — `onnxruntime`, `transformers`, `numpy` absent from `sys.modules` after import. GREEN: `default_session_factory`/`default_tokenizer_factory`, lazy `importlib.import_module` (LC11/VL8).
