...
- [x] 1.5 RED: `supports()` True for `BackendId("onnxruntime")` on both classes, False for `llama_cpp`/`vllm`; explicit both-classes-identical assertion (OR5's discriminator). GREEN: implement `backend_id`/`supports()` on `_OnnxBackendBase` (LC12/VL4).
- [x] 1.6 RED: three `acquire()`s -> three distinct `session_id`s, session-factory counter exactly 1, tokenizer-factory counter exactly 1 (OR2). GREEN: `_OnnxResidency` dataclass (session/tokenizer/input_names/refcount), `acquire()` construct-or-reuse + refcount under `self._lock`.
...
- [x] 2.1 RED: concurrent `run()` on one shared session — two sessions from one Backend, stub `run()` blocks on `threading.Barrier(2)`, `asyncio.gather(embed(s1,...), embed(s2,...))`, both enter before either exits; a lock anywhere times it out (OR3). GREEN: implement `_infer()` calling `residency.session.run(...)` inside `asyncio.to_thread`, no lock held.
- [x] 2.2 RED: provably off the event loop — stub `run()` blocks on a `threading.Event`, a concurrent coroutine advances a counter before the event is set (OR7). GREEN: confirm the single `asyncio.to_thread` hop spans tokenize->run->extract; fix if any blocking work leaks onto the loop.
- [x] 2.3 RED: input filtering — stub tokenizer emits `token_type_ids`, stub session declares only `input_ids`/`attention_mask` -> recorded feed has exactly two keys; reverse case, a declared-but-unemitted input is absent, not synthesized (OR8). GREEN: cache `session.get_inputs()`-derived `input_names` at `acquire()`, filter the feed by intersection.
- [x] 2.4 RED: shape and order — 2-D stub output -> one `Vector` per input in order, equal length, values match; 3-D output -> `OnnxOutputShapeError`; empty input -> empty result, `run` never called (OR9). GREEN: implement `_rows()`, `OnnxOutputShapeError`, `embed()` mapping rows to `Vector`.
- [x] 2.5 RED: `output_name` selection — constructed with `output_name="sentence_embedding"` -> stub records that name in `output_names`; default `None` passed through, `outputs[0]` read (OR9/OR10). GREEN: wire `output_name` construction argument into `_infer()`'s `session.run(self._output_names, feed)` call.
- [x] 2.6 RED: rerank pairs the query — stub tokenizer records `text == [query] * len(documents)`, `text_pair == documents` (OR6). GREEN: implement `rerank()` calling `_infer(session, [query]*len(documents), list(documents))`, mapping `row[0]` to `RerankResult(index=i, score=...)`.
- [x] 2.7 RED: rerank result shape — one `RerankResult` per document, `index` values are `0..n-1` in order (OR9). GREEN: confirm/adjust `rerank()` mapping (falls out of 2.4/2.6 if already correct). (deviation: fell out of 2.4/2.6's implementation directly — `rerank()`'s `enumerate(rows)` mapping already produced ordered `0..n-1` indices, so this RED's tests (`test_rerank_returns_one_result_per_document_with_index_and_score`, `test_rerank_result_indices_are_a_permutation_of_0_to_n_minus_1`) needed no additional production code beyond 2.6's `rerank()` body.)
- [x] 2.8 RED: cancellation is inert — cancel the awaiting task mid-`run()`, `CancelledError` propagates, session untouched, a subsequent `embed()` on the same session still succeeds (OR11). GREEN: confirm no `finally` performs async work on the cancellation path; fix if present. (deviation: no fix was needed — `_infer()` has no `finally` at all, so this RED test passed against 2.1-2.4's existing `_infer()`/`embed()` with zero new production code, confirming OR11's "nothing to clean up" claim structurally rather than by inspection alone.)
- [x] 2.9 Create `tests/integration/test_onnxrt_smoke.py`: opt-in, `pytestmark = skipif(TIBIOS_RAY_ONNX_MODEL/…_TOKENIZER unset)`; real factories, public API only — `embed()` over 3 texts, `rerank()` over 3 ordered documents, two `asyncio.gather`-ed `embed()` calls on one shared session match their serial results (OR3/OR4 empirical discharge), clean `release()`.
- [x] 2.10 `engines/onnxrt.py` module docstring: document OR3/OR4's thread-safety claim citing microsoft/onnxruntime discussions #10107 and #14073, plus accepted limitations (per-Backend-instance residency, artifact-pair debt, no pooling/normalization, no batching, stub cannot prove the real SDK signature, CUDA EP untested).
- [x] 2.11 Local verification: `uv run pytest` (unit green, integration skipped), `uv run ruff check`, `uv run pyright` green for slice 2. Result: `uv run pytest -q` -> 1015 passed, 13 skipped (4 new opt-in `test_onnxrt_smoke.py` tests skipped, env vars unset); `uv run ruff check .` -> All checks passed; `uv run pyright` -> 0 errors, 0 warnings, 0 informations.
