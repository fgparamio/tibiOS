...
### Requirement: RayWorker Upholds Obligations O1-O4 Under Real Concurrency, Verified Via The Shared Harness Against A Fake tibios-ray Server

`RayWorker` MUST uphold O1-O4 (`worker-inbound-port` spec) when driven against a fake `tibios.worker.v1.WorkerExecution` server — no live `tibios-ray` process required in CI. These obligations MUST be verified by invoking the shared O1-O4 conformance harness, not a bespoke `RayWorker`-specific suite.

#### Scenario: RayWorker passes all four obligations against the fake server

- GIVEN the shared O1-O4 harness and a `RayWorker` instance connected to a fake `WorkerExecution` server
...
### Requirement: Client-Side Workload Identity Is Owned Only Until SubmitJob Is Acknowledged

`RayWorker` MUST synchronously register a workload's identity, inside `execute`'s own call (before any `async` suspension point), in a client-local pending-submission table, to satisfy O1/O4 before the server has any record of the workload. This registration MUST be released as soon as `SubmitJob` is acknowledged (successfully or not) — after that point, the server is the sole authority for the workload's lifecycle, and `RayWorker` MUST NOT continue tracking it client-side.

#### Scenario: A cancel issued before SubmitJob is acknowledged is accepted

- GIVEN a `RayWorker::execute` call that has not yet received a `SubmitJob` acknowledgement
- WHEN `cancel` is issued for the same `workload_id`
- THEN it is accepted (`Ok(CancelAck)`), proving the client-side registration happened synchronously

#### Scenario: Client-side tracking ends once SubmitJob succeeds

- GIVEN a `RayWorker::execute` call whose `SubmitJob` has been acknowledged
- WHEN the client-side pending-submission table is inspected
- THEN the workload is no longer present — the server is now the sole authority for it
