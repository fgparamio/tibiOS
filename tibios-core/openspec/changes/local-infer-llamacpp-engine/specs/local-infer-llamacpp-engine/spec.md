...
This engine is CPU-only, deliberately: no GPU, Metal, CUDA, or ROCm acceleration path exists or is planned for this capability — acceleration belongs to the platform's primary (TensorRT-LLM) engine, not this compatibility engine (`local-infer-llamacpp-engine/proposal.md` D6). There is no scenario below asserting the absence of GPU code: an absence is not independently testable behavior, and CPU-only-ness follows from no such path ever being introduced, not from a check that finds one missing.
