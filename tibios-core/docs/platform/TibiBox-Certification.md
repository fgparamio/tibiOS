...
| llama.cpp | `local-infer` | 🔶 assumed | 🔶 assumed | Implemented (`local-infer-llamacpp-engine`, 2026-08-07). No GPU-*accelerated compute* by design (`n_gpu_layers(0)`, `local-infer-llamacpp-engine/spec.md`) — a GPU backend (e.g. Metal) may still compile in and initialize on some host platforms, but performs zero decode work there, confirmed by a real run. Model-free Tier 1/2 tests (build, link, and clean-rejection paths) are green everywhere CI runs. All 4 Tier-3 end-to-end decode tests (`-- --ignored`) passed on 2026-08-07 against a real GGUF model, but on Apple Silicon (dev hardware, not a certification target) — still not yet run on x86_64 or Jetson Orin, the two target platforms above; blocked on operator access to that hardware, not on implementation. |
